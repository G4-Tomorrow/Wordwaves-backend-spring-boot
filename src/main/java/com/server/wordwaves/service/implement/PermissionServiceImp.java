...
public class PermissionServiceImp implements PermissionService {
    ...
    public PaginationInfo<List<PermissionResponse>> getPermissions(int pageNumber, int pageSize, String sortBy, String sortDirection, String searchQuery) {
        ...
        return PaginationUtils.getAllEntities(pageNumber, pageSize, sortBy, sortDirection, searchQuery, validSortByFields,
                pageable -> {
                    if (MyStringUtils.isNotNullAndNotEmpty(searchQuery)) return permissionRepository.findByNameContainingOrDescriptionContaining(searchQuery, searchQuery, pageable);
